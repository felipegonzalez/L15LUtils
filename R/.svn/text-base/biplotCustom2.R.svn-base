biplotCustom2 <- function (x, choices = 1L:2L, scale = 1, pc.biplot = FALSE, 
    ind = TRUE,...) 
{
    if (length(choices) != 2) 
    ...
    xlab = "", ylab = "", ind = TRUE, ejes = FALSE, ...) 
{
    ...
    if (ind) {
        plot(x, cex = cex[1L], xlim = xlim, ylim = ylim, col = col[1L], pch = 16, 
            xlab = xlab, ylab = ylab, ...)
        pointLabel(x, labels = xlabs, cex = cex[1L], col = col[1L], ...)
        # text(x, xlabs, cex = cex[1L], col = col[1L], ...)
    }
    if (var.axes.2) 
        arrows(0, 0, x[, 1L] * 0.8, x[, 2L] * 0.8, col = col[1L], 
            length = arrow.len)
    par(new = TRUE)
    plot(y, axes = FALSE, xlim = xlim * ratio, ylim = ylim * 
        ratio, xlab = "", ylab = "", type = "n", col = col[2L], ...)
    axis(3, col = col[2L], ...)
    axis(4, col = col[2L], ...)
    box(col = col[1L])
    pointLabel(y, labels = ylabs, cex = cex[2L], col = col[2L], ...)
